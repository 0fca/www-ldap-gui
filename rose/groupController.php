...
include_once('models/userModel.php');
    ...
    final class GroupController{
        ...
        static public function addGroup($model){
            ...
            //echo var_dump($entry['member']);
            $retVal = ldap_add(self::$ldap_connection, 'ou='.$model->getName().','.gdn, $entry) ? A100: '<p class="errMsg">'.A400.'</p>';
            self::closeConnection();
            return $retVal;
        }
        ...
        static public function deleteGroup($deleteModel){
            self::prepareConnection();
            $dn = "ou=".$deleteModel->getName().",".gdn;
            $res = ldap_delete(self::$ldap_connection, $dn);
            self::closeConnection();
            $message = $res ? D100 : '<p class="errMsg">'.D400.'</p>';
            return $message;
        }
